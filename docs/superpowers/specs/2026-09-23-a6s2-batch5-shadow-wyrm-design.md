...
5. **lightning_orb 生成池**：`resources/biomes.json` 火山 `enemy_pool` +`lightning_orb` **且** `resources/world/volcano.json` 池同步 +1（双源同改）。
...
**实机验收点**（最短路径）：监狱常规层（shadow_stalker 池出）、深渊常规层（shadow_stalker/shadow_assassin）、深渊挑战房第2波（night_stalker）、火山挑战房（poison_wyrm 波）、火山常规层（lightning_orb 池出）。

## §4 风险与边界

- **虫形覆盖键名拼错** → 静默回落 blob：sheet 评审肉眼可查 + 生成脚本断言必命中覆盖表。
- **双源池不同步**（biomes vs world/volcano）→ validator 池引用检查，两处同改必过。
- **night_stalker spear 字符**：dagger/spear 均已定义，无 KeyError 新面。
...
- lightning_orb 火山池双源接入（biomes + world/volcano）。
